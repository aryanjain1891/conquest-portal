...
def run():
    df = pd.read_excel("scripts/Portal.xlsx",sheet_name="Alumni") #change file path and sheet name
    ...
    for field in list_of_records:
        try:
            ...
            user = User.objects.create(username=field['Name'],email=field.get('Email',None))
